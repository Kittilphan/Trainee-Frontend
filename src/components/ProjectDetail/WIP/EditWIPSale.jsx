...
export default function EditWIP({ wip, filePO, customerId }) {
  const { isLoadingUser, getUserFilter } = useContext(AllUsersContext);
  const [users, setUsers] = useState([]);
  const { wipId, projectId } = useParams();
  ...
  const handleChange = (e) => {
    ...
    if (id === 'status') {
      ...
      SwalConfirm({
        ...
        callbackConfirm: () => {
          setWIPData((prevWIPData) => ({ ...prevWIPData }));
        }
      });
      ...
      if (id.includes('.')) {
        ...
        if (itemIndex !== -1) {
          const updatedArray = [
            ...
            {
              ...prevWIPData[arrayName][itemIndex],
  ...
  const handleSubmit = async (e) => {
    ...
    const file = ['saleFileDelivery', 'fileDeleted'];
    if (wipData.status === 1) {
      ...
      if (wip.dateAssigned) {
        ...
        if (dateAssignedArray.length === 3) {
          const dateAssigned = new Date(dateAssignedArray[0], dateAssignedArray[1] - 1, dateAssignedArray[2]);
          const formattedTodayArray = formattedToday.split('-');
          const todayDate = new Date(formattedTodayArray[0], formattedTodayArray[1] - 1, formattedTodayArray[2]);
      ...
      wipData.dateOnProject = wipData.dateOnProject;
    }

    if (!wipData.wipNumber) {
      SwalDialog({
        icon: 'error',
        text: 'WIP Number is required.'
      });
      return;
    }
    ...
    const formatTypes = (types) => {
      ...
      return Object.entries(groupedTypes)
        ...
        .map(([key, values]) => {
          ...
          return values[checkedKey] ? values[key] : key.replace(/^(eng|cab|data)/, '');
        });
    };

    const formattedWIPData = {
      ...wipData,
      typeEngineer: formatTypes(wipData.typeEngineer),
      typeCabling: formatTypes(wipData.typeCabling),
      typeDataCenter: formatTypes(wipData.typeDataCenter)
    };

    const formattedWIPData2 = {
      ...initial,
      typeEngineer: formatTypes(initial.typeEngineer),
      typeCabling: formatTypes(initial.typeCabling),
      typeDataCenter: formatTypes(initial.typeDataCenter)
    };
    ...
    for (const key in formattedWIPData) {
      if (formattedWIPData.hasOwnProperty(key) && !file.includes(key)) {
        let value = formattedWIPData[key];
        let valueInitial = formattedWIPData2[key];
    ...
    try {
      ...
      if (updatedWIPData?.typeEngineer || updatedWIPData?.typeCabling || updatedWIPData?.typeDataCenter) {
        const roles = [
        ...
        SwalConfirm({
          ...
          callbackConfirm: async () => {
            try {
              const result = await WIPController.updateWIP({
                id: wipId,
                payload: formData,
  ...
  useEffect(() => {
    if (wip) {
      ...
      const typeEngineer = {
        ...defaultTypes,
        ...wip.typeEngineer?.reduce((acc, type) => {
          if (defaultTypes.hasOwnProperty(type)) {
            acc[type] = true;
          } else {
            acc.AnotherChecked = true;
            acc.Another = type;
          }
          return acc;
        }, {})
      ...
      const typeCabling = {
        ...defaultTypes,
        ...wip.typeCabling?.reduce((acc, type) => {
          if (defaultTypes.hasOwnProperty(type)) {
            acc[type] = true;
          } else {
            acc.AnotherChecked = true;
      ...
      const typeDataCenter = {
        ...defaultTypes,
        ...wip.typeDataCenter?.reduce((acc, type) => {
          if (defaultTypes.hasOwnProperty(type)) {
            acc[type] = true;
          } else {
            acc.AnotherChecked = true;
      ...
      const updateWIPDataWithIncomingData = (incomingData) => {
        return {
          wipNumber: incomingData.wipNumber,
          wipName: incomingData.wipName,
          filePO: incomingData.filePO,
          poChecked: incomingData.poChecked,
          xoChecked: incomingData.xoChecked,
          dateOnProject: incomingData.dateOnProject,
          expectedCompleteDate: incomingData.expectedCompleteDate,
          siteName: incomingData.siteName,
          siteAddress: incomingData.siteAddress,
          dateTimeRequest: incomingData.dateTimeRequest,
          customerContactPerson: incomingData.customerContactPerson,
          customerContactTel: incomingData.customerContactTel,
          requireInternalKickoff: incomingData.requireInternalKickoff,
          requestSub: incomingData.requestSub,
          subDetail: incomingData.subDetail,
          typeEngineer: [
            { engImplement: typeEngineer.Implement },
            { engTraining: typeEngineer.Training },
            { engConsult: typeEngineer.Consult },
            { engDelivery: typeEngineer.Delivery },
            { engAnotherChecked: typeEngineer.AnotherChecked },
            { engAnother: typeEngineer.Another }
          ],
          typeCabling: [
            { cabImplement: typeCabling.Implement },
            { cabTraining: typeCabling.Training },
            { cabConsult: typeCabling.Consult },
            { cabDelivery: typeCabling.Delivery },
            { cabAnotherChecked: typeCabling.AnotherChecked },
            { cabAnother: typeCabling.Another }
          ],
          typeDataCenter: [
            { dataImplement: typeDataCenter.Implement },
            { dataTraining: typeDataCenter.Training },
            { dataConsult: typeDataCenter.Consult },
            { dataDelivery: typeDataCenter.Delivery },
            { dataAnotherChecked: typeDataCenter.AnotherChecked },
            { dataAnother: typeDataCenter.Another }
          ],
          saleFileDelivery: incomingData.saleFileDelivery,
          offeredDetails: incomingData.offeredDetails,
  ...
  return (
    <main>
      ...
      <section className="main-content p-5 rounded-4 border shadow">
            ...
            <div className="d-flex flex-wrap py-2">
              ...
              <div className="d-row">
                <select
                  ...
                  style={{ width: "9rem" }}
                  value={filePO.some(po => po._id === wipData.filePO) ? wipData.filePO : ""}
                  id="filePO"
                  onChange={handleChange}
                >
                  ...
                  <div className="d-row">
                    {wipData.filePO && (
                      <div className="ms-4 my-1 fs-5">
                        <ul>
                          {filePO && filePO.filter(po => po._id === wipData.filePO).map((po) => (
                            <li key={po._id}>
          ...
          >
            <div className="d-flex py-2 justify-content-between flex-wrap">
              ...
              <input
                ...
                value={getFormattedDate(wipData.expectedCompleteDate)}
                onChange={handleChange}
              />
            </div>
          </div>
        ...
        <div
          name="Content-field-2"
          className="d-flex flex-column flex-xl-row mt-4 justify-content-between"
        >
          <div name="Content-field-2-left" className="mb-4 me-xl-4">
            ...
            <div className="d-flex flex-column flex-xl-row fs-5 py-2">
              <label
                htmlFor="siteName"
                className="form-label me-xl-2 mb-2 mb-xl-0"
            ...
            <div className="d-flex flex-column flex-xl-row fs-5 py-2">
              <label
                className="me-xl-2 mb-2 mb-xl-0"
                style={{ minWidth: "14.5rem" }}
              >
                วันติดตั้ง
              </label>
              <input
                ...
                className="form-control"
                style={{ width: "10rem" }}
                value={getFormattedDate(wipData.dateTimeRequest)}
                onChange={handleChange}
              />
            </div>
            <div className="d-flex flex-column flex-xl-row fs-5 py-2">
              <label
                htmlFor="customerContactPerson"
                className="form-label me-xl-2 mb-2 mb-xl-0"
                style={{ minWidth: "14.5rem" }}
              >
          ...
          <div name="Content-field-2-right" className="mb-4">
            ...
            <div className="form-check py-2 fs-5">
              <input
                ...
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="requireInternalKickoff">WIP kickoff</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="requestSub">Request Sub</label>
              <AutoResizingTextarea
          ...
          <div name="Content-field-3-left" className="d-row px-0 me-xl-4">
            ...
            <div className="form-check py-2 fs-5">
              <input
                ...
                value=""
                checked={wipData.typeEngineer?.[0]?.engImplement}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeEngineer.engImplement">Implement</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeEngineer?.[1]?.engTraining}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeEngineer.engTraining">Training</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeEngineer?.[2]?.engConsult}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeEngineer.engConsult">Consult</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeEngineer?.[3]?.engDelivery}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeEngineer.engDelivery">Delivery</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeEngineer?.[4]?.engAnotherChecked}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeEngineer.engAnotherChecked">อื่น ๆ</label>
              <input
                className="form-control mt-2"
                type="text"
                id="typeEngineer.engAnother"
                value={wipData.typeEngineer?.[5]?.engAnother}
                onChange={handleChange}
                disabled={
                  !wipData.typeEngineer?.find((e) => "engAnotherChecked" in e)
                    ?.engAnotherChecked
                }
              />
            </div>
          ...
          <div name="Content-field-3-mid" className="d-row px-0">
            ...
            <div className="form-check py-2 fs-5">
              <input
                ...
                value=""
                checked={wipData.typeCabling?.[0]?.cabImplement}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeCabling.cabImplement">Implement</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeCabling?.[1]?.cabTraining}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeCabling.cabTraining">Training</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeCabling?.[2]?.cabConsult}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeCabling.cabConsult">Consult</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeCabling?.[3]?.cabDelivery}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeCabling.cabDelivery">Delivery</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeCabling?.[4]?.cabAnotherChecked}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeCabling.cabAnotherChecked">อื่น ๆ</label>
              <input
                className="form-control mt-2"
                type="text"
                id="typeCabling.cabAnother"
                value={wipData.typeCabling?.[5]?.cabAnother}
                onChange={handleChange}
                disabled={
                  !wipData.typeCabling?.find((e) => "cabAnotherChecked" in e)
                    ?.cabAnotherChecked
                }
              />
            </div>
          ...
          <div name="Content-field-3-right" className="d-row px-0">
            ...
            <div className="form-check py-2 fs-5">
              <input
                ...
                value=""
                checked={wipData.typeDataCenter?.[0]?.dataImplement}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeDataCenter.dataImplement">Implement</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeDataCenter?.[1]?.dataTraining}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeDataCenter.dataTraining">Training</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeDataCenter?.[2]?.dataConsult}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeDataCenter.dataConsult">Consult</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeDataCenter?.[3]?.dataDelivery}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeDataCenter.dataDelivery">Delivery</label>
            </div>
            <div className="form-check py-2 fs-5">
              <input
                className="form-check-input"
                ...
                value=""
                checked={wipData.typeDataCenter?.[4]?.dataAnotherChecked}
                onChange={handleChange}
              />
              <label className="form-check-label" htmlFor="typeDataCenter.dataAnotherChecked">อื่น ๆ</label>
              <input
                className="form-control mt-2"
                type="text"
                id="typeDataCenter.dataAnother"
                value={wipData.typeDataCenter?.[5]?.dataAnother}
                onChange={handleChange}
                disabled={
                  !wipData.typeDataCenter?.find(
                    (e) => "dataAnotherChecked" in e
                  )?.dataAnotherChecked
                }
              />
            </div>
        ...
        <div name="Content-field-4" className="d-row mt-4">
          ...
          <div className="d-flex flex-column flex-lg-row fs-5 py-2 justify-content-between">
            <label htmlFor="offeredDetails" className="form-label me-2" style={{ minWidth: "9rem" }}>
              Offered Details
            </label>
            <AutoResizingTextarea
              className="form-control"
          ...
          <div className="d-flex flex-column flex-lg-row fs-5 py-2 justify-content-between">
            <label htmlFor="remark" className="form-label me-2" style={{ minWidth: "9rem" }}>
              Remark
            </label>
            <AutoResizingTextarea
              className="form-control"
          ...
          <div className="d-row mt-4">
            <label className="fs-4 py-2">เอกสาร (Sale)</label>
            <div className="d-flex flex-column flex-md-row fs-5 py-2">
              ...
              <div>
                <Dropzone
                  ...
                  callbackUploadFile={(file) => handleUploadFile(file, "saleFileDelivery")}
                />
                <FileList
                  file={wipData.saleFileDelivery}
                  callbackRemoveFile={(file) => handleRemoveFile(file, "saleFileDelivery")}
                />
              </div>
            </div>
          </div>
